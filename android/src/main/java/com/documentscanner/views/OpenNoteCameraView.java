...
public class OpenNoteCameraView extends JavaCameraView implements PictureCallback {
    ...
    public void focusCamera(MotionEvent touchEvent) {
        if (mCamera != null ) {
            ...
            if (parameters.getMaxNumMeteringAreas() > 0){
                ...
                if (touchEvent != null) {
                    focusRect = this.calculateFocusArea(touchEvent.getX(), touchEvent.getY());
                }
                focusAreas.add(new Camera.Area(focusRect, 1000));
                parameters.setFocusAreas(focusAreas);
    
                mCamera.setParameters(parameters);
                mCamera.autoFocus(manualFocusCallback);
            }else {
                mCamera.autoFocus(manualFocusCallback);
            }
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_UP) {
            float x = event.getX() * 2000/mSurfaceView.getWidth() - 1000;
            float y = event.getY() * 2000/mSurfaceView.getHeight() - 1000;
            this.focusCamera(event);
        }
        return true;
    }

    public Rect calculateFocusArea(float x, float y) {
        int xCam = (int) (x * 2000/mSurfaceView.getWidth() - 1000);
        int yCam = (int) (y * 2000/mSurfaceView.getHeight() - 1000);
        int FOCUS_AREA_SIZE = 25;
        int left = xCam - FOCUS_AREA_SIZE;
        int top = yCam - FOCUS_AREA_SIZE;
        int right = xCam + FOCUS_AREA_SIZE;
        int bottom = yCam + FOCUS_AREA_SIZE;

        return new Rect(
            left < -1000 ? -1000 : left,
            top < -1000 ? -1000 : top,
            right > 1000 ? 1000 : right,
            bottom > 1000 ? 1000 : bottom
        );
    }

    private Camera.AutoFocusCallback manualFocusCallback = new Camera.AutoFocusCallback() {
        @Override
        public void onAutoFocus(boolean success, Camera camera) {
            if (success) {
                ...
                Log.d(TAG, "SUCCESS focus");

            } else {
                Log.d(TAG, "FAIL focus");
            }
        }
    };
